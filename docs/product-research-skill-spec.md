...
These are full-workflow target criteria, not a claim that the current repository can already perform every listed stage. The current repository implements the source-agnostic orchestration control plane and the explicit deterministic scoring and analytical decision boundaries described above; provider-backed research acquisition, score generation, Red Team, reporting, and end-to-end evaluation remain unavailable.
