...
The scoring decision executor receives the resulting decision-facing state directly. Initial Scoring may independently consume the structured Risk result for the Risk & Compliance dimension, but it never derives or changes this state. Missing or malformed state is fail-closed as `RISK REVIEW`; it does not inspect Evidence text or infer a state from scores.

During Red Team, the Agent/caller may rerun the authoritative Risk analysis and submit complete before/after `RiskComplianceResult` values. `product_research/red_team_revision.py` compares only their retained `risk_gate`, requires declared current-run causal Evidence, and preserves the trace; it does not accept a raw gate override or rerun Risk logic.
...
After that stage, pass the complete immutable `UnitEconomicsResult` to [product_research/initial_scoring.py](../product_research/initial_scoring.py) for the retained Contribution Margin rubric, then pass the resulting scorecard and the same complete result to [product_research/scoring_decision.py](../product_research/scoring_decision.py). Initial Scoring and the scoring executor do not recalculate margins, rerun either economics gate, or generate economics thresholds.

During Red Team, the Agent/caller may rerun Unit Economics and submit complete before/after results. The deterministic revision boundary compares both retained Gate outcomes and `EconomicsOutcome` only when both policy thresholds remain equal and current-run causal Evidence is declared; it does not recalculate economics or accept raw Gate/outcome overrides.
